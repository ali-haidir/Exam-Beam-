...
class Worker2(QThread):
    ...
    # def update_warning(self, addr, client_socket):


    def get_warning(self, port):
        ...
        # print("Thread Id: ", str(thread.name))
        ...
        while True:
            try:
                if client_socket:
                    ...
                    self.Warning += int(data)

                    print('Warning Count: ', self.Warning)
        ...
        t2 = threading.Thread(target=self.get_warning, args=(p2,))
        t2.start()
...
#         x = 5
    ...
    #         print("window 4 = " , self.student_window_4.pixmap())
        if x>=4:
            if x>=4 and self.student_window_1.pixmap() == None:
